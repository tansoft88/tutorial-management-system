	...
	if($this->request->is('post')){
		...
		//debug($ecnumber);die();
			...
			if ($this->Auth->login() && $grpmbr == 3){//HR Admin assistant
				...
				$this->redirect($this->Auth->loginRedirect = array('controller'=>'Users', 'action'=> 'index_reports',$entryPointID,$check_in_out));
			}
		...
		if($this->Session->read('Auth.User.username') != null){
			...
			if ($this->Auth->login() && $grpmbr == 3){//HR Admin assistant
				$this->redirect($this->Auth->loginRedirect = array('controller' => 'Users', 'action'=> 'index_reports'),$entryPointID);
			}
			elseif ($this->Auth->login() && $grpmbr == 4){//Check In and Out User
			//debug($nguva);die();
				/***save current logged in user***/
